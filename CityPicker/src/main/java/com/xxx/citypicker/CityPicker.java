package com.sanjiliandong.citypicker;
...
public class CityPicker implements PopupWindow.OnDismissListener, View.OnClickListener {
    ...
    public CityPicker(Activity context, View parent, String textFromAssets) {
        mContext = context;
        mParent = parent;
        //初始化选择器
        ...
        pickerView.findViewById(R.id.city_picker_tv_exit).setOnClickListener(this);
        pickerView.findViewById(R.id.city_picker_tv_ok).setOnClickListener(this);
        ...
        initData(textFromAssets);
    }

    /**
     * 初始化城市数据
     */
    private void initData(String textFromAssets) {
        mDatas = new Gson().fromJson(textFromAssets, new TypeToken<List<Province>>() {
        }.getType());
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.city_picker_tv_ok) {
            if (mOnCitySelectListener != null) {
                Province province = mDatas.get(mProvinceWheel.getSelected());
                City city = !province.getCities().isEmpty() ? province.getCities().get(mCityWheel.getSelected()) : null;
                String provinceName = province.getAreaName();
                String cityName = city == null ? "" : city.getAreaName();
                String countyName;
                if (city == null || mCountyWheel.getSelected() >= city.getCounties().size()) {
                    countyName = "";
                } else {
                    countyName = city.getCounties().get(mCountyWheel.getSelected()).getAreaName();
                }
                mOnCitySelectListener.onCitySelect(provinceName, cityName, countyName);
                mPickerWindow.dismiss();
            }
        } else if (v.getId() == R.id.city_picker_tv_exit) {
            mPickerWindow.dismiss();
        }
    }
